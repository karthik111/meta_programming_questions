...
def count_of_nodes(root, queries, s):
    ...
    for q in queries:
        ...
        c = count_nodes(node, q[0], q[1], s)
        out.append(c)
    return out

def find_node(root, node_val):
    if root.val == node_val:
        return root
    for child in root.children:
        sub_node = find_node(child, node_val)
        if sub_node != None and sub_node.val == node_val:
            return sub_node

def count_nodes(node, node_val, c, s):
    count = 0
    if s[node.val-1] == c:
        count +=1
    for child in node.children:
        count += count_nodes(child, node_val, c, s)
